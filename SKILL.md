...
name: boss-recruit
version: 1.0.0
description: BOSS直聘招聘助手。处理"新招呼"收件箱（附件优先）：有附件简历的候选人零弹窗下载 PDF 并同步到飞书多维表格，没附件的主动发消息要简历。内置反风控节奏层（随机延时/批量上限/验证码熔断/权益耗尽诊断/防发错人）。当用户提到 Boss直聘招聘、处理新招呼、候选人简历同步飞书、批量打招呼要简历时使用。
metadata:
  openclaw:
    requires:
      bins: [node, opencli]
...
把 Boss 招聘端的"新招呼收件箱"自动化（**附件优先**）：**有附件简历 → 零弹窗下载 PDF 同步飞书人才库；没附件 → 主动发消息要简历**，全程走反风控节奏层模拟真人，避免封号。

> 核心目的是把**附件简历**收进人才库——**同步只针对附件 PDF**。在线简历只在"没附件、要发消息"时读一下，用作打招呼的**称呼与上下文**，**不写入飞书**。有附件的直接下载存档，没附件的发消息催，等下一轮自动收。
...
scenarios/
  setup.mjs            首次配置飞书目标表
  inbox.mjs            场景1编排: recommend→去重→attachment→(有:同步PDF / 无:要简历)→控节奏→汇报
adapters/boss/
